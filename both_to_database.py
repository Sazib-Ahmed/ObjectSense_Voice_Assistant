            ...
            for box, track_id, class_id in zip(boxes, track_ids, class_ids):
                if class_id in moving_object_ids or class_id in stationary_object_ids:
                    ...
                    try:
                        ...
                        if existing_record:
                            cursor.execute(
                                "UPDATE detections SET class_id = %s, class_name = %s, class_type = %s, x = %s, y = %s, width = %s, height = %s, timestamp = %s WHERE tracker_id = %s",
                                (class_id, class_name, class_type, x_value, y_value, w_value, h_value, datetime.now(), track_id)
                            )
                        else:
                            cursor.execute(
                                "INSERT INTO detections (tracker_id, class_id, class_name, class_type, x, y, width, height, timestamp) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                                (track_id, class_id, class_name, class_type, x_value, y_value, w_value, h_value, datetime.now())
                            )
                        db.commit()
                    except Error as e:
