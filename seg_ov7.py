...
# Filter the results for moving and stationary objects
...
for result in results:
    ...
    for class_index in moving_object_ids + stationary_object_ids:
        if class_index in seg_class_indices:
            seg_class = seg_classes[class_index]
            obj_indices = torch.where(clss == class_index)
            obj_masks = masks[obj_indices]
            obj_mask = torch.any(obj_masks, dim=0).int() * 255
            filtered_class_masks[seg_class] = obj_mask
            filtered_mask_areas[seg_class] = torch.sum(obj_mask).item()

            obj_bbox = boxes[obj_indices].squeeze(0)
            if obj_bbox.ndim == 1:  # In case of a 1D tensor
                filtered_bounding_boxes[seg_class] = obj_bbox.unsqueeze(0)
            else:
                filtered_bounding_boxes[seg_class] = obj_bbox
...
process_overlaps(filtered_class_masks, filtered_mask_areas, filtered_bounding_boxes)
